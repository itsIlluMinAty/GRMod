...
function MatchmakingStatus( elMatchStatus )
{
	...
	var k_fakeMmStartedAtKey = 'ui_fake_matchmaking_started_at';
	...
	var _SearchPanelSearching = function( lobbySettings )
	{
		...
		var searchStatus = _GetSearchStatus();
		elLabel.text = searchStatus && searchStatus.charAt( 0 ) === '#' ? $.Localize( searchStatus ) : searchStatus;
	...
	var _UpdateSearchTime = function()
	{
		...
		if ( GameInterfaceAPI.GetSettingString( k_fakeMmActiveKey ) === '1' )
		{
			var startedAt = Number( GameInterfaceAPI.GetSettingString( k_fakeMmStartedAtKey ) );
			seconds = startedAt > 0 ? Math.max( 0, Math.floor( Date.now() / 1000 ) - startedAt ) : 0;
		}
